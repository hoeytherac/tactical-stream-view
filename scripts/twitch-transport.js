...
export class TwitchTransport {
  ...
  async authorize(clientId, showCode, signal) {
    ...
    while (!this.stopped && !signal.aborted && Date.now() < expires) {
      ...
      const response = await this.fetcher("https://id.twitch.tv/oauth2/token", {
        ...
        body: new URLSearchParams({ client_id: this.clientId, scopes: SCOPES, device_code: device.device_code, grant_type: "urn:ietf:params:oauth:grant-type:device_code" })
      });
      const data = await response.json();
      if (response.ok) {
        if (this.stopped || signal.aborted) break;
  ...
  openSocket(url, previous) {
    ...
    socket.onmessage = async ({ data }) => {
      ...
      try {
        ...
        if (type === "session_welcome") {
          ...
          this.fail("Twitch revoked the chat subscription. Reconnect.");
    ...
    socket.onclose = () => {
      clearTimeout(watchdog);
      if (this.sockets.delete(socket) && !this.stopped) this.fail("Twitch disconnected. Reconnect to resume.");
    };
